...
/**
 * Get most recent forecast generated before a cutoff time (ISO string)
 * Useful to retrieve a "day_0" that was generated before the daily window ended.
 */
export async function getMostRecentForecastBefore(cutoffIso: string): Promise<{
  id: string;
  nwsIssuedAt: string;
  llmGeneratedAt: string;
  nwsForecastText: string;
  ...
  try {
    const result = await sql`
      ...
      FROM forecasts
      WHERE source = 'fresh_llm'
        AND llm_generated_at < ${cutoffIso}
      ORDER BY llm_generated_at DESC
      LIMIT 1
    `;
    ...
    console.error('[FORECAST-STORAGE] Failed to query forecast before cutoff:', error);
    return null;
  }
}
