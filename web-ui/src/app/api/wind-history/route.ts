...
function convertToWindDataPoints(measurements: RawWindMeasurement[]): WindDataPoint[] {
  ...
  hourlyGroups.forEach((hourMeasurements, groupKey) => {
    ...
    // Create timestamp anchored to the most recent measurement in this hour
    const latestMeasurement = hourMeasurements.reduce((latest, current) => {
      const latestDate = Date.UTC(latest.year, latest.month - 1, latest.day, latest.hour, latest.minute);
      const currentDate = Date.UTC(current.year, current.month - 1, current.day, current.hour, current.minute);
      return currentDate > latestDate ? current : latest;
    }, hourMeasurements[0]);

    const latestGmt = new Date(Date.UTC(
      latestMeasurement.year,
      latestMeasurement.month - 1,
      latestMeasurement.day,
      latestMeasurement.hour,
      latestMeasurement.minute
    ));

    // Get Pacific time for the latest measurement in the hour
    const pacificTimestamp = formatInTimeZone(latestGmt, PACIFIC_TIMEZONE, "yyyy-MM-dd'T'HH:mm:ssXXX");
    // Use hour start for labeling to align with training examples (10:00-18:00 buckets)
    // Display label as end-of-hour to match UI (hour+1), while keeping hour field as start
    const endHour = (hour + 1) % 24;
    const endDisplayHour = endHour === 0 ? 24 : endHour; // handle midnight rollover
    const timeStr = format(new Date().setHours(endDisplayHour, 0, 0, 0), 'h a');
  ...
  return dataPoints
    ...
    .filter(point => point.hour >= 10 && point.hour <= 17); // Align with training (10:00-18:00 → hours 10-17)
}
  ...
  try {
    ...
    try {
      const { data, etag, cached } = await createEtagCache<WindHistoryResponse>(
        fileCache,
        cacheKey,
        5 * 60 * 1000 // 5 minute cache to keep current hour fresh
      )(
        request,
        async () => {
          ...
          const response = await fetchWithTimeout(NOAA_DATA_URL, {
            headers: {
