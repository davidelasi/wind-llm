...
export async function storeWindActuals(dataPoints: WindDataPoint[]): Promise<number> {
  try {
    ...
    for (const point of dataPoints) {
      // Validate hour is within training-aligned window (10-17)
      if (point.hour < 10 || point.hour > 17) {
        console.warn(`[WIND-ACTUALS] Skipping invalid hour: ${point.hour} (must be 10-17)`);
        continue;
      }
