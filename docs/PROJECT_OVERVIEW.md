...
**Selection Logic (issuance-based, 4 buckets):**
- We pick the example file using the NWS issuance time (Pacific), not the current clock.
- Buckets:
  - `fc1`: 00:00–05:59 PST
  - `fc2`: 06:00–11:59 PST
  - `fc3`: 12:00–17:59 PST
  - `fc4`: 18:00–23:59 PST
- Example: Issued Dec 16 at 12:47 PM PST → month `dec` and `fc3` → load `dec_fc3_examples.json`.
- The same bucket assignment is stored with the forecast metadata to keep prompt and storage aligned.
